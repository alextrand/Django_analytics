...
# cl (clearences) - перехваты
# 6y (shot 6 yards) - удары с шести ярдов
# pa (shot from penalty area)  - удары с зоны пенальти
# ob (shot out of box) - удары за пределы ворот
# sht - удары в створ ворот
# r = rating, tp = passes_total, ap = passes_accurate, kp = passes_key, cl = clearences, 6y = shot 6 yards, result = winner
...
#         self.r_h = data[col_dict['r_h'] - 1]
#         self.r_a = data[col_dict['r_a'] - 1]
#         self.tp_h = data[col_dict['tp_h'] - 1]
#         self.tp_a = data[col_dict['tp_a'] - 1]
#         self.ap_h = data[col_dict['ap_h'] - 1]
#         self.ap_a = data[col_dict['ap_a'] - 1]
#         self.kp_h = data[col_dict['kp_h'] - 1]
#         self.kp_a = data[col_dict['kp_a'] - 1]
#         self.cl_h = data[col_dict['cl_h'] - 1]
#         self.cl_a = data[col_dict['cl_a'] - 1]
#         self.sy_h = data[col_dict['sy_h'] - 1]
#         self.sy_a = data[col_dict['sy_a'] - 1]
#         self.pa_h = data[col_dict['pa_h'] - 1]
#         self.pa_a = data[col_dict['pa_a'] - 1]
#         self.ob_h = data[col_dict['ob_h'] - 1]
#         self.ob_a = data[col_dict['ob_a'] - 1]
...
#         # for (за)
#         r = count_for(matches, 'r', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 1).value = r
#         tp = count_for(matches, 'tp', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 2).value = tp
#         ap = count_for(matches, 'ap', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 3).value = ap
#         kp = count_for(matches, 'kp', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 4).value = kp
#         sy = count_for(matches, 'sy', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 5).value = sy
#         pa = count_for(matches, 'pa', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 6).value = pa
#         ob = count_for(matches, 'ob', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 7).value = ob
#         g = count_for(matches, 'g', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 8).value = g
#         cl = count_for(matches, 'cl', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 9).value = cl
#         # against (против)
#         r = count_against(matches, 'r', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 10).value = r
#         tp = count_against(matches, 'tp', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 11).value = tp
#         ap = count_against(matches, 'ap', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 12).value = ap
#         kp = count_against(matches, 'kp', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 13).value = kp
#         sy = count_against(matches, 'sy', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 14).value = sy
#         pa = count_against(matches, 'pa', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 15).value = pa
#         ob = count_against(matches, 'ob', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 16).value = ob
#         g = count_against(matches, 'g', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 17).value = g
#         cl = count_against(matches, 'cl', teams[i], last_matches_report1)
#         ws.cell(dest_row_rep1 + i, dest_col_rep1 + 18).value = cl
#
#
...
#     write_file('top18.xlsx', 'AW3', 'BR50', 5, 16)
